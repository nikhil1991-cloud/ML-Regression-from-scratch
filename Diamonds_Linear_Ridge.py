...
#CF = sum(y-y_predicted)^2 + lambda(sum(beta^2))
#y_predicted = beta*x and lambda is the lagrange multiplier
#We will use Gradient Descent method, calculate the derivative of CF and update weights
# An efficient way is to iterate over different values of lambda, calculate the r2 score and select the lambda for the best r2 score.
...
for l in range (0,len(lm)):
 ...
 for e in range (0,epochs):
        ...
        del_beta_GD[:,l] = ((2/N)*(np.matmul(-X_train.T,(y_train-Y_pred)) + lmda))
        CF[e,l] = (np.sum((y_train-Y_pred)**2)+lmda*np.sum(beta_GD[:,l]**2))/(2*len(y_train))
        beta_GD[:,l] = beta_GD[:,l] - alpha*del_beta_GD[:,l]
        Y_p = np.matmul(X_test,beta_GD[:,l])
        r2_score[l] = metrics.r2_score(y_test,Y_p)

#Instead of doing a gradient descent, there is also a closed form solution to ridge regression which is similar to the closed form
#solution of OLS without regularization.
#the coefficients betas can be estimated by, betas = (X^T*X + lambda)^-1(X^T*Y)
beta_closed = np.zeros((np.shape(X_train)[1],len(lm)))
CF_closed = np.zeros((epochs,len(lm)))
r2_score_closed = np.zeros(len(lm))
l=0
for l in range (0,len(lm)):
    lmda = lm[l]
    beta_closed[:,l] = np.matmul(np.linalg.inv(np.matmul(X_train.T,X_train) + lmda),np.matmul(X_train.T,y_train))
    Y_p = np.matmul(X_test,beta_closed[:,l])
    r2_score_closed[l] = metrics.r2_score(y_test,Y_p)
